...
يوجد نظام لتجميد ميزات معينة في لوحة التحكم بحيث لا يستطيع مستخدم لوحة التحكم الوصول إليها.
- **ملف التحكم**: `client/src/lib/feature-flags.ts`
- لتجميد ميزة: اجعل قيمتها `true`
- لإزالة التجميد: اجعل قيمتها `false`
- الميزات المدعومة حالياً: `heroSlider` (صور السلايدر)، `discounts` (الخصومات)
- الميزات المجمدة تظهر بأيقونة قفل وشفافية في القائمة الجانبية
- عند محاولة الوصول للصفحة مباشرة عبر الرابط تظهر رسالة "ميزة مجمّدة"
...
- الصور يتم رفعها من الجهاز مباشرة وتُخزن في Supabase Storage (bucket: uploads) مع fallback محلي في `/public/uploads`
...
### 28 أكتوبر 2025
- ✅ **إضافة نظام البلدات وأجور التوصيل**
  - إنشاء جدول towns في قاعدة البيانات مع دعم الوحدات (أغورة)
  - صفحة `/admin/towns` لإدارة البلدات وأجور التوصيل
  - تكامل اختيار البلدة في صفحة Checkout
  - حساب تلقائي لأجرة التوصيل وإضافتها للمجموع الكلي
  - بيانات أولية لـ 5 بلدات رئيسية (الناصرة، حيفا، عكا، سخنين، شفاعمرو)
  - إصلاح مشكلة تحويل الوحدات (شيكل ↔ أغورة) في نماذج الإدخال

- ✅ **نظام الخصومات الزمنية للمنتجات**
  - إضافة حقول `discountedPrice`, `discountStartDate`, `discountEndDate` لجدول المنتجات
  - صفحة `/admin/discounts` لإدارة خصومات المنتجات
  - عرض مرئي للخصومات في Storefront:
    - Badge أحمر يوضح نسبة الخصم
    - السعر الأصلي مشطوب باللون الرمادي
    - السعر المخفض باللون الأخضر
  - استخدام السعر الفعلي (مع الخصم إن وُجد) في سلة الطلبات
  - نظام ذكي للتحقق من تواريخ سريان الخصم
